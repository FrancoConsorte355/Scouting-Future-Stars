...
def seccion_modelo():
    ...
    # Renombrar la columna UPER
    talentos_predichos.rename(columns={'UPER': 'Player Efficiency Rating'}, inplace=True)

    # Interfaz interactiva
    st.subheader("Top 5 Jugadores con Mayor Player Efficiency Rating")
    if not talentos_predichos.empty:
        top_5 = talentos_predichos.nlargest(5, 'Player Efficiency Rating')[['player_name', 'edad', 'Player Efficiency Rating']]
        st.table(top_5)

    if st.checkbox("Mostrar todos los jugadores menores de 21 años"):
        st.dataframe(talentos_predichos[['player_name', 'edad', 'Player Efficiency Rating']])

    # Gráfico interactivo
    if not talentos_predichos.empty:
        st.subheader("Gráfico de Player Efficiency Rating por Jugador")
        st.bar_chart(talentos_predichos.set_index('player_name')['Player Efficiency Rating'])
    else:
        st.write("No hay datos disponibles para mostrar el gráfico.")
